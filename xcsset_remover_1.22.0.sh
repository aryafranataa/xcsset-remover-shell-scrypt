...
#
# Options:
#   --deep            Enable thorough scanning of entire $HOME directory (may take long time)
#   --folder PATH     Enable thorough scanning within a specific folder
#   --include-allhook When used with --folder, also scan hook-named files
#                     outside .git/hooks within the specified folder
#
# IMPORTANT: Always run with sudo. XCSSET changes project.pbxproj
# ownership to root — without sudo the script cannot read or clean
...
#   Use --deep to recursively scan the entire home directory ($HOME)
#   with maxdepth 100. This may take several minutes or longer but will
#   catch malware in custom folder structures like ~/Learning/ or anywhere
#   else in the home directory. A confirmation prompt will appear before
#   the deep scan begins.
#
# --folder option:
#   Use --folder PATH to recursively scan only the specified directory
#   for Xcode projects and git repository hooks. This mode is intended as
#   a folder-only check for downloaded projects and does not perform
#   the full system persistence scan of LaunchAgents, daemons, defaults,
#   fake apps, or other global artifacts.
#   Add --include-allhook together with --folder to also scan files whose
#   names match standard Git hook names even when they are outside
#   .git/hooks inside the target folder.
#
# What this script does:
#   1. fix_pbxproj_ownership — restores root-owned .pbxproj to user
#   2. Scan phase — 14 parallel modules detect all XCSSET artifacts
#   3. Phase A  — removes file-system threats (shell configs, pbxproj
#      payloads, LaunchAgents/Daemons, git hooks, hidden files, etc.)
#   4. Phase B  — kills injection agents and C2-related processes
#   5. Phase C  — resets TCC privacy permissions hijacked by XCSSET
#   6. Blocks known C2 domains via /etc/hosts
#
# pbxproj cleanup passes:
#   Pass 0 — find malicious build phase UUIDs (shellScript/script
#            patterns + "Provision Target Device" name fallback)
#   Pass 1 — remove entire malicious build phase blocks + cross-refs
#   Pass 2 — remove random-key payload entries from XCBuildConfiguration
#            buildSettings (e.g. AF17F99, AZ17F89, AP17I99 = "((...))")
#            and any shellScript/script lines referencing them
#   Pass 3 — restore ENABLE_USER_SCRIPT_SANDBOXING = YES
#
# v22 changes from v21:
# - NEW: default/general scan now includes focused duplicated/trojanized .app
#   detection in common locations such as /Applications, ~/Applications,
#   ~/Downloads, ~/Desktop, ~/Documents, ~/Library/Caches,
#   ~/Library/Application Support, ~/Library/Application Scripts,
#   /Users/Shared, and /private/tmp. This catches fake Apple/System app
#   copies without requiring --deep.
# - ENHANCED: --deep keeps the broader maxdepth 100 app bundle scan across
#   the home folder and system-wide writable locations.
#
# v21 changes from v20:
# - CRITICAL FIX: shell config cleaner now uses a dedicated Python
#   detector/remover instead of only grep -vE. This catches quoted,
#   parenthesized, backgrounded XCSSET one-liners such as:
#   "((echo <base64> | base64 --decode | sh) >/dev/null 2>&1 &)"
#   in .zshrc/.zprofile/.bashrc/etc.
# - FIX: exact XCSSET base64 bootstrap payloads are detected even when
#   wrapped with extra spaces, quotes, or shell redirection.
# - SAFETY: shell config cleanup writes a backup into evidence before
#   changing the file and only replaces the file when suspicious content
#   was actually removed.
# - QUALITY: added v1.22.0 documentation and clearer quick/default,
#   --deep, and --folder mode behavior.
#
# v20 changes from v19:
# - NEW FEATURE: --folder option for targeted thorough scanning.
#   Use --folder PATH to recursively scan only the specified folder
#   for Xcode projects and git repository hooks with maxdepth 100.
# - NEW EVIDENCE: folder and deep scans now write scanned_folders.txt
#   in the evidence directory to record the exact folder coverage.
# - PATCH: --include-allhook extends --folder mode so it also scans
#   standard Git hook filenames found outside .git/hooks within the
#   specified folder. This option is only valid together with --folder.
# - NEW FEATURE: --deep option for thorough home directory scanning.
#   By default, scanning is limited to common development directories
#   (~Developer/, ~Documents/, ~Desktop/, ~Downloads/, ~Dropbox/).
#   The --deep option enables recursive scanning of the entire $HOME
#   directory with maxdepth 100 to catch malware in custom folder
#   structures like ~/Learning/ or ~/Projects/. Includes user
#   confirmation prompt with visual warning about long execution time.
#   Non-interactive mode auto-proceeds with warning message.
#
# v19 changes from v18:
# - CRITICAL FIX: XCSSET changes project.pbxproj ownership to root:wheel
#   with mode 600, making files unreadable by the normal user. Both the
#   scan phase (grep) and cleanup phase (Python open()) silently failed —
#   root-owned infected files were reported as CLEAN or FAILED, and the
#   malware inside XCBuildConfiguration buildSettings was never checked.
#   Fixed by adding fix_pbxproj_ownership() that runs BEFORE scanning:
#   finds all root-owned project.pbxproj via `find -user root` and
#   restores user ownership via sudo chown/chmod. Python cleaner also
#   has a sudo-cat read fallback for any files that remain unreadable.
# - FIX: pbxproj cleaner restores ENABLE_USER_SCRIPT_SANDBOXING = YES
#   in cleaned projects. XCSSET forces this to NO so its injected shell
#   scripts can execute without sandbox restrictions (Pass 3).
#
# v17 changes from v14:
# - CRITICAL FIX: scan_shell() never detected any .zshrc/.bashrc
#   infection. The global IFS=$'\n\t' (no space) caused the space-
#   separated FILES variable to become a single concatenated string.
#   The for-loop ran once, [ -f ] failed, scan always reported CLEAN.
#   Fixed by using newline-separated paths.
# - FIX: .zshrc removal regex now includes `| base64 --decode`.
# - FIX: pbxproj cleaner catches "Provision Target Device" by name,
#   even when shellScript is empty or cleared.
# - FIX: pbxproj cleaner adds catch-all `base64 --decode ... | sh`
#   without requiring `shellScript = "` context.
# - FIX: pbxproj cleaner handles PBXBuildRule `script` field and
#   standalone `sh -c "${HEXKEY}"` on multi-line continuations,
#   including project.pbxproj remnants left after the base64 echo
#   payload line has already been cleaned or removed.
#
...
hidden_files,fake_apps,mainscpt_infected,tmp_payloads,agent_evidence,deep_file_scan,duplicated_apps}
...
DUP_APP_LST="$TMP/duplicated_apps.lst"
DEEP_INJECTED_FILES_LST="$TMP/deep_injected_files.lst"
...
touch "$PROJECTS_LST" "$AGENTS_LST" "$DAEMONS_LST" "$HOOKS_LST" \
      "$HIDDEN_LST" "$XCSSET_BUNDLE_LST" "$TROJAN_APP_LST" "$DUP_APP_LST" "$DEEP_INJECTED_FILES_LST" "$TMP_PAYLOAD_LST" \
...
# Deep mode: scan many file types across the Mac for XCSSET-like injected code.
# Indonesia:
#   Dipakai hanya saat --deep. Fokus mencari kemungkinan payload XCSSET yang
#   disisipkan ke file lain di komputer, bukan hanya .zshrc / project.pbxproj.
# English:
#   Used only with --deep. Focuses on possible XCSSET payloads injected into
#   other files on the computer, not only .zshrc / project.pbxproj.
scan_deep_file_injections() {
  [ "${DEEP_SCAN:-0}" -eq 1 ] || return 0
  rlog "=== Deep File Injection Scan ==="
  scan_detail hidden "deep scanning files for injected code..."
  local DEEP_CANDIDATES="$TMP/deep_file_candidates.lst"
  > "$DEEP_CANDIDATES"

  # Keep this list focused on text/source/config files. Avoid full binary grep.
  # Indonesia: folder berat/noisy dikecualikan agar scan tetap aman dan usable.
  # English: heavy/noisy folders are excluded to keep the scan safe and usable.
  find "$REAL_HOME" -maxdepth 100 -type f \
    \( -name ".zshrc" -o -name ".zprofile" -o -name ".bashrc" -o -name ".bash_profile" -o -name ".profile" \
       -o -name "*.sh" -o -name "*.command" -o -name "*.zsh" -o -name "*.bash" \
       -o -name "*.rb" -o -name "*.py" -o -name "*.js" -o -name "*.ts" -o -name "*.swift" \
       -o -name "*.m" -o -name "*.mm" -o -name "*.c" -o -name "*.cpp" -o -name "*.h" \
       -o -name "*.plist" -o -name "*.xcscheme" -o -name "*.pbxproj" -o -name "*.xcconfig" \
       -o -name "Podfile" -o -name "Gemfile" -o -name "Makefile" -o -name "package.json" \
       -o -name "*.json" -o -name "*.yml" -o -name "*.yaml" -o -name "*.env" \) \
    -not -path "*/Library/Developer/Xcode/DerivedData/*" \
    -not -path "*/node_modules/*" -not -path "*/Pods/*" -not -path "*/.Trash/*" \
    -not -path "*/Library/Caches/*" -not -path "*/xcsset_evidence*" \
    -size -5M 2>/dev/null | sort -u > "$DEEP_CANDIDATES"

  local TOTAL; TOTAL=$(wc -l < "$DEEP_CANDIDATES" | tr -d ' ')
  local CUR=0
  while IFS= read -r F; do
    [ -f "$F" ] || continue
    CUR=$((CUR+1))
    [ $((CUR % 20)) -eq 0 ] && scan_tick hidden "deep file $CUR/$TOTAL"

    # Strong indicators: encoded shell pipeline, defaults random-domain loader,
    # xxd reverse pipe to shell, curl pipe shell, or Xcode build-phase injection.
    if LC_ALL=C grep -Iq . "$F" 2>/dev/null && \
       LC_ALL=C grep -nE \
       'echo[[:space:]]+[A-Za-z0-9+/]{30,}={0,2}[[:space:]]*\|[[:space:]]*base64[[:space:]]+(--decode|-d|-D)[[:space:]]*\|[[:space:]]*(env[[:space:]]+[A-Za-z_][A-Za-z0-9_]*=)?[[:space:]]*(/bin/)?(ba)?sh|defaults[[:space:]]+read[[:space:]]+[a-z0-9]{5,8}[[:space:]]+[A-Za-z0-9_]{5,32}|xxd[[:space:]]+-p[[:space:]]+-r[[:space:]]*\|[[:space:]]*(/bin/)?(ba)?sh|curl[^\n]+\|[[:space:]]*(/bin/)?(ba)?sh|Provision Target Device|AF17F99|adobestats|flixprice|xcassets.*bash|mainscpt|main\.scpt.*base64' \
       "$F" > "$TMP/deep_match.tmp" 2>/dev/null; then
      local SAFE; SAFE=$(printf '%s' "${F#$REAL_HOME/}" | tr '/ :' '___' | cut -c1-180)
      cp "$F" "$EVIDENCE_DIR/deep_file_scan/${SAFE}" 2>/dev/null
      cp "$TMP/deep_match.tmp" "$EVIDENCE_DIR/deep_file_scan/${SAFE}.matches.txt" 2>/dev/null
      printf '%s\n' "$F" >> "$DEEP_INJECTED_FILES_LST"
      add_finding "$TIER_CONFIRMED" RED hidden "Deep injected code: $(basename "$F")" "$F"
    fi
  done < "$DEEP_CANDIDATES"
  rm -f "$DEEP_CANDIDATES" "$TMP/deep_match.tmp" 2>/dev/null
}

# General + Deep mode: detect duplicated/trojanized app bundles.
# Indonesia:
#   XCSSET sering meniru nama aplikasi Apple atau menanam main.scpt/applet di
#   bundle .app. Mulai v1.22.0, general scan juga mengecek app duplikat pada
#   lokasi umum. Mode --deep tetap melakukan pencarian lebih luas/recursive.
# English:
#   XCSSET often mimics Apple app names or plants main.scpt/applet inside .app
#   bundles. Starting v1.22.0, the default/general scan also checks duplicated
#   apps in common locations. --deep still performs broader recursive scanning.
scan_duplicated_apps() {
  rlog "=== Duplicated / Trojanized App Bundle Scan ==="
  local APP_LIST="$TMP/duplicated_apps_scan.lst"
  > "$APP_LIST"

  if [ "${DEEP_SCAN:-0}" -eq 1 ]; then
    scan_detail fakeapps "deep scanning duplicated .app bundles..."
    # Indonesia: --deep mencari di lokasi yang lebih luas dengan maxdepth besar.
    # English: --deep searches broader locations with larger maxdepth.
    for ROOT in "$REAL_HOME" /Applications /private/tmp /tmp /Users/Shared /Library/Application\ Support; do
      [ -d "$ROOT" ] || continue
      find "$ROOT" -maxdepth 100 -name "*.app" -type d \
        -not -path "*/.Trash/*" \
        -not -path "*/Library/Developer/Xcode/DerivedData/*" \
        -not -path "*/node_modules/*" \
        -not -path "*/Pods/*" \
        -not -path "*/xcsset_evidence*" 2>/dev/null >> "$APP_LIST"
    done
  else
    scan_detail fakeapps "general scanning duplicated .app bundles..."
    # Indonesia: general scan dibuat lebih cepat, tetapi tetap mencakup lokasi
    # umum tempat XCSSET / malware sering menaruh app duplikat atau fake app.
    # English: general scan stays fast, but now covers common locations where
    # XCSSET / malware often drops duplicated or fake app bundles.
    for ROOT in \
      /Applications \
      "$REAL_HOME/Applications" \
      "$REAL_HOME/Downloads" \
      "$REAL_HOME/Desktop" \
      "$REAL_HOME/Documents" \
      "$REAL_HOME/Library/Caches" \
      "$REAL_HOME/Library/Application Support" \
      "$REAL_HOME/Library/Application Scripts" \
      /Users/Shared \
      /private/tmp \
      /tmp; do
      [ -d "$ROOT" ] || continue
      case "$ROOT" in
        /Applications) MAX_APP_DEPTH=1 ;;
        /private/tmp|/tmp) MAX_APP_DEPTH=4 ;;
        *) MAX_APP_DEPTH=6 ;;
      esac
      find "$ROOT" -maxdepth "$MAX_APP_DEPTH" -name "*.app" -type d \
        -not -path "*/.Trash/*" \
        -not -path "*/Library/Developer/Xcode/DerivedData/*" \
        -not -path "*/node_modules/*" \
        -not -path "*/Pods/*" \
  ...
  local TOTAL; TOTAL=$(wc -l < "$APP_LIST" | tr -d ' ')
  local CUR=0
  while IFS= read -r APP; do
    [ -d "$APP" ] || continue
    CUR=$((CUR+1))
    [ $((CUR % 10)) -eq 0 ] && scan_tick fakeapps "app bundle $CUR/$TOTAL"
    local ANAME; ANAME=$(basename "$APP")
    local SCPT="$APP/Contents/Resources/Scripts/main.scpt"
    local REASON=""
    local INFECTED=0

    # Indonesia: app Apple/System yang muncul di lokasi writable/non-System
    # sangat mencurigakan karena sering dipakai sebagai kamuflase.
    # English: Apple/System app names appearing in writable/non-System paths
    # are suspicious because they are commonly used as camouflage.
    if { [ -d "/System/Applications/$ANAME" ] || [ -d "/System/Applications/Utilities/$ANAME" ]; } && \
       ! printf '%s' "$APP" | grep -qE '^/System/Applications'; then
      INFECTED=1; REASON="Apple/System app name duplicated outside /System"
    fi

    # Indonesia: cek main.scpt karena XCSSET sering memakai AppleScript dropper.
    # English: check main.scpt because XCSSET often uses AppleScript droppers.
    if [ "$INFECTED" -eq 0 ] && [ -f "$SCPT" ] && check_mainscpt_infected "$SCPT"; then
      INFECTED=1; REASON="infected main.scpt in app bundle"
    fi

    # Indonesia: cek script/plist di dalam app bundle untuk pola shell encoded.
    # English: check scripts/plists inside app bundles for encoded shell patterns.
    if [ "$INFECTED" -eq 0 ] && [ -d "$APP/Contents" ]; then
      if find "$APP/Contents" -maxdepth 5 -type f -size -5M \
        \( -name "*.sh" -o -name "*.command" -o -name "*.plist" -o -name "*.js" -o -name "*.scpt" -o -name "*.applescript" \) \
        -exec grep -Iq . {} \; \
        -exec grep -qE 'base64[[:space:]]+(--decode|-d|-D).*[|][[:space:]]*(/bin/)?(ba)?sh|defaults[[:space:]]+read[[:space:]]+[a-z0-9]{5,8}|xxd[[:space:]]+-p[[:space:]]+-r|curl.*[|][[:space:]]*(/bin/)?(ba)?sh|osascript.*do shell script' {} \; -print 2>/dev/null | grep -q .; then
        INFECTED=1; REASON="suspicious script/plist inside app bundle"
      fi
    fi

    # Indonesia: heuristic app dengan signing ad-hoc + bundle id acak.
    # English: heuristic for ad-hoc signed app with random-looking bundle id.
    if [ "$INFECTED" -eq 0 ] && check_ad_hoc_random_bundleid "$APP"; then
      INFECTED=1; REASON="ad-hoc signed with random-looking bundle identifier"
    fi

    if [ "$INFECTED" -eq 1 ]; then
      local SAFE_NAME; SAFE_NAME=$(printf '%s' "$ANAME" | tr ' /():' '_____')
      local SUFFIX="general"
      [ "${DEEP_SCAN:-0}" -eq 1 ] && SUFFIX="deep"
      collect_app_evidence "$APP" "$EVIDENCE_DIR/duplicated_apps/${SAFE_NAME}_${SUFFIX}"
      printf '%s\n' "$APP" >> "$DUP_APP_LST"
      printf '%s\n' "$APP" >> "$TROJAN_APP_LST"
      add_finding "$TIER_CONFIRMED" RED fakeapps "Duplicated/trojan app: $ANAME ($REASON)" "$APP"
    fi
  done < "$APP_LIST"
  rm -f "$APP_LIST" 2>/dev/null
}
...
DUPAPP_COUNT=$(wc -l < "$DUP_APP_LST"       | tr -d ' ')
DEEPFILE_COUNT=$(wc -l < "$DEEP_INJECTED_FILES_LST" | tr -d ' ')
...
if [ "$TOTAL_THREATS" -gt 0 ]; then
  ...
  N_DUPAPP=$(_n "$DUP_APP_LST")
  N_DEEPFILE=$(_n "$DEEP_INJECTED_FILES_LST")
